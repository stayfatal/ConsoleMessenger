...
func (dm *DbManager) CreateUser(user models.User) (int, error) {
	var id int
	err := dm.db.QueryRow("insert into users (username,password)  ($1,$2) returning id", user.Username, user.Password).Scan(&id)
	return id, errors.Wrap(err, "inserting into users")
}

func (dm *DbManager) GetUserByName(username string) (models.User, error) {
...
func (dm *DbManager) GetUserById(id int) (models.User, error) {
	user := models.User{}
...
func (dm *DbManager) GetUsernameById(id int) (string, error) {
	var username string
